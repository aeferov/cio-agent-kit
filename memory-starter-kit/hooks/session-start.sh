...
last_handoff=$(ls -t "$MEM"/*handoff*.md "$MEM"/handoff_*.md 2>/dev/null | sed -n 1p)
...
if [ -n "$href" ] && [ -n "$head_now" ] && [ "$href" != "$head_now" ]; then
  ...
  echo "    Это работа других сессий или другой машины. Что изменилось:"
  git log --oneline "$href..HEAD" 2>/dev/null | sed 's/^/      /'
  echo "    Прочитай дельту ДО утверждений о состоянии проекта."
fi
